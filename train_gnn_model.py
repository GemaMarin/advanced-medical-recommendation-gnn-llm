...
# ---------------------- Modelo --------------------------
class GNN(nn.Module):
    def __init__(self, hidden_channels, dropout=0.3, num_layers=3):
        super().__init__()
        ...
        for _ in range(num_layers - 2):
            self.convs.append(SAGEConv(hidden_channels, hidden_channels))
            self.bns.append(nn.BatchNorm1d(hidden_channels))
        self.convs.append(SAGEConv(hidden_channels, 128))  # Reducción a 128
...
def sample_pos_neg_for_margin(pos_scores, neg_scores, device='cpu'):
    pos_count, neg_count = pos_scores.size(0), neg_scores.size(0)
    if pos_count > neg_count:
        pos_scores = pos_scores[torch.randperm(pos_count, device=device)[:neg_count]]
    elif neg_count > pos_count:
        neg_scores = neg_scores[torch.randperm(neg_count, device=device)[:pos_count]]
...
def train_epoch_ranking(model, data, optimizer, device='cpu', calc_recall_topk=None):
    ...
    pos_scores = pred[ground_truth == 1]
    neg_scores = pred[ground_truth == 0]
    pos_scores, neg_scores = sample_pos_neg_for_margin(pos_scores, neg_scores, device)
    y = torch.ones(neg_scores.size(0), device=device)
    ranking_loss = F.margin_ranking_loss(pos_scores, neg_scores, y, margin=1.0)
    ...
    recall_at_k = compute_recall_at_k(probs, labels, k=calc_recall_topk) if calc_recall_topk else None
...
def evaluate_ranking(model, data, device='cpu', calc_recall_topk=None):
    ...
    pos_scores, neg_scores = sample_pos_neg_for_margin(pos_scores, neg_scores, device)
    y = torch.ones(neg_scores.size(0), device=device)
    val_loss = F.margin_ranking_loss(pos_scores, neg_scores, y, margin=1.0)
    probs = torch.sigmoid(pred).detach().cpu().numpy()
    labels = ground_truth.detach().cpu().numpy()
    auc = roc_auc_score(labels, probs) if len(np.unique(labels)) > 1 else 0.0
    acc = accuracy_score(labels, (probs >= 0.5))
...
def train_loop_ranking(model, train_data, val_data, epochs=100, lr=1e-4, weight_decay=1e-4,
