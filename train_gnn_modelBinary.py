# ---------------------- CAMBIOS CLAVE ----------------------
# - Se elimina Margin Ranking Loss y se usa BCEWithLogitsLoss
# - Se ajustan funciones de entrenamiento y evaluación para usar predicciones completas
# -----------------------------------------------------------
...
class GNN(nn.Module):
    def __init__(self, hidden_channels, dropout=0.3, num_layers=3):
        ...
        for _ in range(num_layers - 2):
            self.convs.append(SAGEConv(hidden_channels, hidden_channels))
            self.bns.append(nn.BatchNorm1d(hidden_channels))
        self.convs.append(SAGEConv(hidden_channels, 128))
...
def train_epoch_bce(model, data, optimizer, device='cpu', calc_recall_topk=None):
    ...
    optimizer.zero_grad()
    pred = model(data)
    labels = data["Drug", "DrugHasIndication", "Disorder"].edge_label.float()
...
# ---------------- Loop principal BCE ----------------
def train_loop_bce(model, train_data, val_data, epochs=100, lr=1e-4, weight_decay=1e-4,
                   model_file="gnn_model_bce.pth", device='cpu',
